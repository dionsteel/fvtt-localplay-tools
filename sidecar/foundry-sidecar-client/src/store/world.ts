...
export const useWorldStore = defineStore(
  ...
  () => {
    ...
    async function getActorData(id: string, force = false) {
      if (!force) {
        return (actorData.value[id] = actorData.value[id] || (await config.fetchJson(`/actor/${id}`)));
      } else {
        return (actorData.value[id] = await config.fetchJson(`/actor/${id}`));
      }
    }
    ...
    return {
      ...
      currentWorldActors,
      worldId,
      get allSelectedActors() {
        return config.SelectedActors;
      },
      worldActorMap,
      startup,
      started,
      ...
      addSelectedActor(actorId: string) {
        ...
        if (worldId.value) {
          ...
          config.addSelectedActor(actorId, worldId.value, { ...listing });
          config.$persist();

          // if (listing) {
          //   if (existingIdx < 0) {
          //     config.SelectedActors.push({ worldId: worldId.value, actorId, listing: { ...listing } });
          //     console.log("adding selected actor id", actorId, config.SelectedActors);
          //   } else {
          //     config.SelectedActors[existingIdx].listing = listing;
          //     console.error("faile dto add", actorId);
          //   }
          //   config.$persist();
          // } else {
