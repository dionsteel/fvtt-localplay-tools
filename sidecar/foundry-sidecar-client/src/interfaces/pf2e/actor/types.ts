...
import * as ItemInstance from "../item/index";
import { EffectTrait } from "../item/abstract-effect";
import { ItemInstances } from "../item";
// import { any } from "@/interfaces/pf2e/canvas/index.ts";
import { ActorPF2e } from "../actor";
import { ItemPF2e } from "../item";
// import { any } from "@scene/index.ts";
// import { immunityTypes, resistanceTypes, weaknessTypes } from "../";
import { DamageRoll, Rolled } from "../system/roll";
import { CheckDC, DegreeOfSuccessString } from "../system/degree-of-success";
import { PredicatePF2e } from "../system/predication";
import { StatisticCheck } from "../system/statistic/index";
import { StrikeData, TraitViewData } from "./data/base";
import { ModifierPF2e } from "./modifiers";
import { ABILITY_ABBREVIATIONS, DC_SLUGS, MOVEMENT_TYPES, SAVE_TYPES, SKILL_ABBREVIATIONS, SKILL_LONG_FORMS, UNAFFECTED_TYPES } from "./values";
import { SetElement } from "../item/class";
import { immunityTypes, resistanceTypes, weaknessTypes } from "../config/iwr";
/** Used exclusively to resolve `ActorPF2e#isOfType` */
interface ActorInstances {
  character: ActorInstance.CharacterPF2e;
  creature: ActorInstance.CreaturePF2e;
  familiar: ActorInstance.FamiliarPF2e;
  hazard: ActorInstance.HazardPF2e;
  loot: ActorInstance.LootPF2e;
  party: ActorInstance.PartyPF2e;
  npc: ActorInstance.NPCPF2e;
  vehicle: ActorInstance.VehiclePF2e;
}
type EmbeddedItemInstances<TParent extends ActorPF2e> = {
  [K in keyof ItemInstances<TParent>]: ItemInstances<TParent>[K][];
};
type AttributeString = SetElement<typeof ABILITY_ABBREVIATIONS>;
...
type AttackItem = ItemInstance.WeaponPF2e | ItemInstance.MeleePF2e | ItemInstance.SpellPF2e;
...
interface CheckContextParams<TStatistic extends StatisticCheck | StrikeData = StatisticCheck | StrikeData, TItem extends AttackItem | null = AttackItem | null>
  extends RollContextParams<TStatistic, TItem> {
  targetedDC: string;
}
interface DamageRollContextParams<TStatistic extends StatisticCheck | StrikeData | null = StatisticCheck | StrikeData | null, TItem extends AttackItem | null = AttackItem | null>
  extends RollContextParams<TStatistic, TItem> {
  /** An outcome of a preceding check roll */
  outcome?: DegreeOfSuccessString | null;
}
interface CheckContext<TActor extends ActorPF2e, TStatistic extends StatisticCheck | StrikeData = StatisticCheck | StrikeData, TItem extends AttackItem | null = AttackItem | null>
