...
export interface ClassItemLink extends ItemLink {
  level: number;
}

export interface ClassDefensesPF2e {
  unarmored: number;
  light: number;
  medium: number;
  heavy: number;
...
export interface ClassDataPF2e extends ItemDataPF2e {
  items: Record<string, ClassItemLink>;
  keyAbility: { value: AbilityScoreKeyPF2e[] };
  hp: number;
  trainedSkills: { value: string[]; additional: number };
  perception: number;
  savingThrows: { fortitude: number; reflex: number; will: number };
  classDC: number;
  attacks: ClassAttacks;
  defenses: ClassDefensesPF2e;
