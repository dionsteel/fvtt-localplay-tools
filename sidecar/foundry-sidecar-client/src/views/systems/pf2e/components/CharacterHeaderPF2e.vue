      ...
      <div class="ability" v-for="(data, ak) in actor?.system?.abilities">
        ...
        <div class="value">
          <small v-if="ak == 'con'" class="fullscore save" title="Saving Throw">
            {{ ["", "T", "E", "M", "L"][actor.system.saves.fortitude.rank] + " " }}+{{ actor.system.saves.fortitude.totalModifier }}<br />
          </small>
          <small v-if="ak == 'dex'" class="fullscore save" title="Saving Throw">
            {{ ["", "T", "E", "M", "L"][actor.system.saves.reflex.rank] + " " }}+{{ actor.system.saves.reflex.totalModifier }}<br />
          </small>
          <small v-if="ak == 'wis'" class="fullscore save" title="Saving Throw">
            {{ ["", "T", "E", "M", "L"][actor.system.saves.will.rank] + " " }}+{{ actor.system.saves.will.totalModifier }}<br />
          </small>
          <template v-for="skil of actor.system.skills">
            <small :class="`skill skill-rank-${skil.rank}`" v-if="skil.ability == ak && skil.rank > 0">{{ capitalize(skil?.label || "") }} +{{ skil.totalModifier }} <br /></small>
          </template>
        </div>
        <!-- {{ data.proficient }}
          {{ data.max }} -->
