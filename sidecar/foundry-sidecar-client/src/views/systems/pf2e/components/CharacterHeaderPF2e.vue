...
import { skullSharp, bedSharp, bulbSharp, medalSharp, addSharp, keySharp } from "ionicons/icons";
    ...
    <IonItem>
      <div class="abilities">
        <div class="ability" v-for="(data, ak) in actor?.system?.abilities">
          ...
          <div class="value">
            <small v-if="ak == 'con'" class="fullscore save">
              {{ ['', 'Trained', 'Expert', 'Master', 'Legend'][actor.system.saves.fortitude.rank] + ', ' }}+{{
                actor.system.saves.fortitude.totalModifier }}<br />
            </small>
            <small v-if="ak == 'dex'" class="fullscore save">
              {{ ['', 'Trained', 'Expert', 'Master', 'Legend'][actor.system.saves.reflex.rank] + ', ' }}+{{
                actor.system.saves.reflex.totalModifier }}<br />
            </small>
            <small v-if="ak == 'wis'" class="fullscore save">
              {{ ['', 'Trained', 'Expert', 'Master', 'Legend'][actor.system.saves.will.rank] + ', ' }}+{{
                actor.system.saves.will.totalModifier }}<br />
            </small>
            <template v-for="skil of actor.system.skills">
              <small class="skill" v-if="skil.ability == ak && skil.rank">{{ capitalize(skil.label) }} +{{ skil.totalModifier }}
                <br /></small>
            </template>
          </div>
          <!-- {{ data.proficient }}
