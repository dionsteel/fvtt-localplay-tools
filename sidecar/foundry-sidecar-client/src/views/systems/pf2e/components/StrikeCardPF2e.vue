  ...
  <IonAccordion>
    <IonItem slot="header">
      ...
      <IonLabel>
        ...
        <IonBadge slot="end" v-for="trait of [...strike?.traits, ...strike.weaponTraits]">{{ capitalize(trait.label || "")
        }}</IonBadge>
      </IonLabel>
    </IonItem>
    <div slot="content" class="ion-padding">
      ...
      <IonItem v-if="strike.ready">
        ...
        <IonButton v-for="(variant, varIdx) of strike.variants" @click="doStrikeVariant(strikeIdx, varIdx)">{{
          variant.label }}</IonButton>
      </IonItem>
      <IonItem>
        <IonButton v-for="(aux, auxIdx) in strike.auxiliaryActions" @click="doStrikeAux(strikeIdx, auxIdx)">{{ aux?.action
      ...
      <br />
      <IonBadge :style="{ margin: '1px 5px' }" v-for="trait of [...strike?.traits, ...strike.weaponTraits]">{{
        capitalize(trait.label || "") }}</IonBadge>
      <IonBadge slot="end" color="tertiary" v-if="strike?.item.system.traits.rarity">{{
        capitalize(strike?.item.system.traits.rarity || "") }}</IonBadge>
