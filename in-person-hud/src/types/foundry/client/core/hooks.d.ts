...
declare global {
  ...
  type HookParamsPreCreateItem = HookParameters<
    "preCreateItem",
    [PreCreate<foundry.documents.ItemSource>, DocumentModificationContext<Actor<TokenDocument<Scene | null> | null> | null>, string]
  >;
  ...
  type HookParamsRenderChatMessage = HookParameters<"renderChatMessage", [ChatMessage, JQuery, foundry.documents.ChatMessageSource]>;
  type HookParamsTargetToken = HookParameters<"targetToken", [User, Token<TokenDocument<Scene>>, boolean]>;
  type HookParamsControlToken = HookParameters<"controlToken", [User, Token<TokenDocument<Scene>>, boolean]>;
  // type HookParamsControlObject = HookParameters<"controlToken", [User, Token<TokenDocument<Scene>>, boolean]>;
  ...
  >;
  type HookParamsControlObject<T extends PlaceableObject, N extends string> = HookParameters<`control${N}`, [T, boolean]>;
  type HookParamsHoverObject<T extends PlaceableObject, N extends string> = HookParameters<`hover${N}`, [T, boolean]>;
  type HookParamsDestroyObject<T extends PlaceableObject, N extends string> = HookParameters<`destroy${N}`, [T]>;
  type HookParamsRefreshObject<T extends PlaceableObject, N extends string> = HookParameters<`refresh${N}`, [T]>;
  type HookParamsDrawObject<T extends PlaceableObject, N extends string> = HookParameters<`draw${N}`, [T]>;
  ...
  type HookParamTypes =
    ...
    | HookParamsControlToken
    | HookParamsUpdate
    | HookParamsControlObject
    | HookParamsHoverObject
    | HookParamsDestroyObject
    | HookParamsRefreshObject
    | HookParamsDrawObject
    | HookParamsUpdateWorldTime;

  export interface HookParamTypeMap {
    controlToken: [User, Token<TokenDocument<Scene>>, boolean];
    preUpdateToken: [
      Scene,
    ...
    updateToken: [Token, DocumentUpdateData<Token>, DocumentModificationContext<Token["parent"]>];
    refreshToken: [Token, DocumentUpdateData<Token>, DocumentModificationContext<Token["parent"]>];
    createToken: [Token, DocumentUpdateData<Token>, DocumentModificationContext<Token["parent"]>];
    preUpdateActor: [Actor];
    updateActor: [Actor, DocumentUpdateData<Actor>, DocumentModificationContext<Actor["parent"]>];
    refreshActor: [Actor, DocumentUpdateData<Actor>, DocumentModificationContext<Actor["parent"]>];
    updateActor: [Actor, DocumentUpdateData<Actor>, DocumentModificationContext<Actor["parent"]>];

    init: [];
    setup: [Game];
    canvasInit: [DrawnCanvas];
    canvasReady: [DrawnCanvas];
    ready: [];
    deleteCombat: [Combat, { [key: string]: unknown }, string];
    dropCanvasData: [Canvas, DropCanvasData];
    getChatLogEntryContext: [JQuery, EntryContextOption[]];
    getSceneControlButtons: [SceneControl[]];
    hotbarDrop: [Hotbar, unknown, string];
    lightingRefresh: [LightingLayer];
    preCreateItem: [
      PreCreate<foundry.documents.ItemSource>,
      DocumentModificationContext<Actor<TokenDocument<Scene | null> | null> | null>,
      string
    ];
    preUpdateCombat: [Combat, object, { diff: boolean; advanceTime: number; [key: string]: unknown }, string];
    preUpdateToken: [
      Scene,
      foundry.documents.TokenSource,
      DeepPartial<foundry.documents.TokenSource>,
    ...
    renderChatMessage: [ChatMessage, JQuery, foundry.documents.ChatMessageSource];
    targetToken: [User, Token<TokenDocument<Scene>>, boolean];
    controlToken: [User, Token<TokenDocument<Scene>>, boolean];
    // `render${N}`: HookCallback< [T, JQuery, ReturnType<T["getData"]>]>;
    renderChatLog: [ChatLog, "ChatLog"];
    renderChatPopout: [ChatPopout, "ChatPopout"];
    renderCombatTrackerConfig: [CombatTrackerConfig, "CombatTrackerConfig"];
    renderCompendiumDirectory: [CompendiumDirectory, "CompendiumDirectory"];
    renderDialog: [Dialog, "Dialog"];
    renderActorDirectory: [ActorDirectory<Actor<null>>, "ActorDirectory"];
    renderItemDirectory: [ItemDirectory<Item<null>>, "ItemDirectory"];
    renderSceneControls: [SceneControls, "SceneControls"];
    renderSettings: [Settings, "Settings"];
    renderTokenHUD: [TokenHUD, "TokenHUD"];
    renderJournalPageSheet: [JournalPageSheet<JournalEntryPage<JournalEntry | null>>, "JournalPageSheet"];
    renderJournalTextPageSheet: [JournalTextPageSheet<JournalEntryPage<JournalEntry | null>>, "JournalTextPageSheet"];
    closeCombatTackerConfig: [CombatTrackerConfig, JQuery];
  }
  ...
  class Hooks {
    ...
    static on(...args: HookParamsUpdate<Token, "Token">): number;
    static on(...args: HookParamsUpdateWorldTime): number;
    static on(...args: HookParameters<string, unknown[]>): number;
    static on<K extends keyof HookParamTypeMap>(hook: K, callback: HookCallback<HookParamTypeMap[K]>);
    /**
